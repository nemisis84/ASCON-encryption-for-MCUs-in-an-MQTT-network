...
forceinline word_t MXORBIC(word_t c, word_t a, word_t b, int i, int ns) {
  ...
  if (ns == 1) {
      EOR_BIC_ROR(c.s[0].w, a.s[0].w, b.s[0].w, 0, tmp, i);
  }

  if (ns == 2) {
      EOR_BIC_ROR(c.s[0].w, a.s[0].w, b.s[0].w, 0, tmp, i);
      EOR_BIC_ROR(c.s[1].w, a.s[1].w, b.s[0].w, -1, tmp, i);
      CLEAR();
      EOR_AND_ROR(c.s[1].w, a.s[1].w, b.s[1].w, 0, tmp, i);
      EOR_AND_ROR(c.s[0].w, a.s[0].w, b.s[1].w, 1, tmp, i);
      CLEAR();
  }

  if (ns == 3) {
      EOR_AND_ROR(c.s[0].w, b.s[0].w, a.s[1].w, 1, tmp, i);
      EOR_BIC_ROR(c.s[0].w, a.s[0].w, b.s[0].w, 0, tmp, i);
      EOR_AND_ROR(c.s[0].w, b.s[0].w, a.s[2].w, 2, tmp, i);

      EOR_AND_ROR(c.s[1].w, b.s[1].w, a.s[2].w, 1, tmp, i);
      EOR_BIC_ROR(c.s[1].w, a.s[1].w, b.s[1].w, 0, tmp, i);
      EOR_AND_ROR(c.s[1].w, b.s[1].w, a.s[0].w, -1, tmp, i);

      EOR_BIC_ROR(c.s[2].w, b.s[2].w, a.s[0].w, -2, tmp, i);
      EOR_ORR_ROR(c.s[2].w, a.s[2].w, b.s[2].w, 0, tmp, i);
      EOR_AND_ROR(c.s[2].w, b.s[2].w, a.s[1].w, -1, tmp, i);
  }

  if (ns == 4) {
      EOR_BIC_ROR(c.s[0].w, a.s[0].w, b.s[0].w, 0, tmp, i);
      EOR_BIC_ROR(c.s[1].w, a.s[1].w, b.s[0].w, -1, tmp, i);
      EOR_BIC_ROR(c.s[2].w, a.s[2].w, b.s[0].w, -2, tmp, i);
      EOR_BIC_ROR(c.s[3].w, a.s[3].w, b.s[0].w, -3, tmp, i);

      EOR_AND_ROR(c.s[1].w, a.s[1].w, b.s[1].w, 0, tmp, i);
      EOR_AND_ROR(c.s[2].w, a.s[2].w, b.s[1].w, -1, tmp, i);
      EOR_AND_ROR(c.s[3].w, a.s[3].w, b.s[1].w, -2, tmp, i);
      EOR_AND_ROR(c.s[0].w, a.s[0].w, b.s[1].w, 1, tmp, i);

      EOR_AND_ROR(c.s[2].w, a.s[2].w, b.s[2].w, 0, tmp, i);
      EOR_AND_ROR(c.s[3].w, a.s[3].w, b.s[2].w, -1, tmp, i);
      EOR_AND_ROR(c.s[0].w, a.s[0].w, b.s[2].w, 2, tmp, i);
      EOR_AND_ROR(c.s[1].w, a.s[1].w, b.s[2].w, 1, tmp, i);

      EOR_AND_ROR(c.s[3].w, a.s[3].w, b.s[3].w, 0, tmp, i);
      EOR_AND_ROR(c.s[0].w, a.s[0].w, b.s[3].w, 3, tmp, i);
      EOR_AND_ROR(c.s[1].w, a.s[1].w, b.s[3].w, 2, tmp, i);
      EOR_AND_ROR(c.s[2].w, a.s[2].w, b.s[3].w, 1, tmp, i);
  }
