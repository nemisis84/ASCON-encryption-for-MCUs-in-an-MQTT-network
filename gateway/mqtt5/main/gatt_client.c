...
#define REMOTE_SERVICE_UUID        0x181A   // Environmental Sensing Service UUID
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        mqtt_publish("/ascon-e2e/data-storage", (const char *)ble_data_buffer, copy_len, t_start);
...
int extract_sequence_number(const uint8_t *data, size_t len) {
    if (!data || len < AD_PATTERN_LEN) {
        return -1;
    }

    int ad_start_index = -1;
    for (int i = len - AD_PATTERN_LEN; i >= 0; --i) {
        if (memcmp(data + i, AD_PATTERN, AD_PATTERN_LEN) == 0) {
            ad_start_index = i;
            break;
        }
    }
    ...
    size_t ad_len = len - ad_start_index;
    char extracted_ad[ad_len + 1];
    memcpy(extracted_ad, data + ad_start_index, ad_len);
    extracted_ad[ad_len] = '\0';

    int seq = -1;
    char sensor_id[20];
    if (sscanf(extracted_ad, "|%[^|]|%d", sensor_id, &seq) == 2) {
        return seq;
    }
...
void ble_forward(uint8_t *data, size_t len, uint64_t t_start) {
    ...
    int seq_num = extract_sequence_number(data, len);
...
void ble_init(void *pvParameters) {
    ...
    // Release memory from Classic Bluetooth (not needed)
    esp_err_t ret = esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT);
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to release classic BT memory: %s", esp_err_to_name(ret));
    }

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to initialize BT controller: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to enable BLE: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to initialize Bluedroid: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to enable Bluedroid: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_ble_gap_register_callback(esp_gap_cb);
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to register BLE GAP callback: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_ble_gattc_register_callback(esp_gattc_cb);
    if (ret) {
        ESP_LOGE(GATTC_TAG, "Failed to register GATT client callback: %s", esp_err_to_name(ret));
        return;
    }
    ...
    ESP_LOGI(GATTC_TAG, "BLE Initialized successfully!");
